...
function searchableText(faq) {
  return [faq.question, faq.summary, ...faq.keywords, ...faq.stepByStep]
    .join(' ')
    .toLowerCase();
}

export default function TutorialCenterScreen({ onClose, onReplay, onShowOnScreen, open, page = false }) {
  ...
  const content = (
      ...
      >
        <header>
          ...
          <button aria-label={page ? 'Back to profile' : 'Close Help and FAQs'} onClick={onClose} type="button">
            {page ? <ArrowLeft /> : <X />}
          </button>
        </header>
              ...
              <article className={isExpanded ? 'is-expanded' : ''} key={faq.id}>
                ...
                {isExpanded && (
                  <div className="pm-help-center__answer">
                    <ol>
                      {faq.stepByStep.map((step) => <li key={step}>{step}</li>)}
                    </ol>
                    <p><strong>Important:</strong> {faq.importantRule}</p>
...
export function HelpCenterPage() {
  ...
  return (
    <TutorialCenterScreen
      ...
      onReplay={() => window.dispatchEvent(new CustomEvent('pm-start-help-tour', { detail: 'welcome' }))}
      onShowOnScreen={(faq) => window.dispatchEvent(new CustomEvent('pm-show-help-guide', { detail: faq }))}
    />
  );
}
