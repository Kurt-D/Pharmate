...
function AdminIcon({ name, size = 19 }) {
  const paths = {
    ...
    orders: <><path d="M3 6h18l-2 13H5L3 6Z" /><path d="M8 10h8M9 3h6" /></>,
    ...
    help: <><circle cx="12" cy="12" r="10" /><path d="M9.5 9a2.6 2.6 0 1 1 4.1 2.1c-1 .72-1.6 1.17-1.6 2.4M12 17h.01" /></>,
    ...
    <div className={`admin-shell${preferences.darkMode ? ' is-dark-mode' : ''}`}>
      <nav className="admin-sidebar">
        <div className="admin-brand">
          <span className="admin-brand-mark"><img alt="PharMate" src={pharmateLogo} /></span>
          <span><strong>PharMate</strong><small>Admin Portal</small></span>
        </div>
        ...
        <button className="admin-minor" onClick={() => updatePreference('darkMode', !preferences.darkMode)} type="button">
          <AdminIcon name={preferences.darkMode ? 'sun' : 'moon'} /> <span>{preferences.darkMode ? 'Light mode' : 'Dark mode'}</span>
        </button>
      ...
      <div className="admin-main">
        <header className="admin-header">
          ...
          <div className="admin-account">
            <span className="admin-avatar">{(user?.name || user?.email || 'Admin').slice(0, 2).toUpperCase()}</span>
            <span>
