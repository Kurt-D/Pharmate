...
export default function Medicines() {
  ...
  const counts = useMemo(
    () =>
      meds.reduce(
        (result, medicine) => {
          result.total += 1;
          result[stockState(medicine.stock_quantity).key] += 1;
          return result;
        },
  ...
  function openEdit(medicine) {
    ...
    setForm({
      ...
      common_strength: medicine.common_strength || '',
      dosage_form: medicine.dosage_form || 'Tablet',
      short_description: medicine.short_description || '',
      rx_class: medicine.rx_class || 'RX',
      stock_quantity: Number(medicine.stock_quantity || 0),
      id: medicine.id,
    });
    setError('');
  }
  ...
  async function saveMedicine(event) {
    ...
    try {
      await api(drawer === 'edit' ? `/api/admin/medicines/${form.id}` : '/api/admin/medicines', {
        method: drawer === 'edit' ? 'PUT' : 'POST',
        body: form,
      });
      setNotice(
        drawer === 'edit'
          ? `${form.generic_name} was updated.`
          : `${form.generic_name} was added to the formulary.`
      );
      setDrawer(null);
      await load();
    } catch (requestError) {
  ...
  async function deleteMedicine() {
    ...
    try {
      ...
      setNotice(`${deleteTarget.generic_name} was removed.`);
  ...
  const summary = [
    ['Total medicines', counts.total, Pill, 'blue'],
    ['In stock', counts.in, PackageCheck, 'green'],
    ['Low stock', counts.low, CircleAlert, 'amber'],
    ['Out of stock', counts.out, PackageX, 'red'],
  ];
  ...
  return (
    <section className={`admin-medicine-workspace${drawer ? ' has-drawer' : ''}`}>
      ...
      {notice && (
        <div className="admin-medicine-message is-success" role="status">
          <ShieldCheck size={18} />
          <span>{notice}</span>
      ...
      <div className="admin-medicine-layout">
        <main>
          <div className="admin-medicine-toolbar">
            <button onClick={openAdd} type="button" className="add">
              <Plus size={17} />
              Add medicine
            </button>
          ...
          <div className="admin-medicine-summary">
          ...
          <section className="admin-medicine-list">
            <header>
              ...
              <span>Low stock threshold: 10 units</span>
            </header>
            <div className="admin-medicine-table-wrap">
              <table>
                <thead>
                  <tr>
                    <th>#</th>
                ...
                <tbody>
                  {loading ? (
                          ...
                          <td>
                            <div className="medicine-name">
                              ...
                              <div>
                                ...
                                {medicine.is_provisional ? (
                                  <small>Pending pharmacist verification</small>
                                ) : (
                                  <small>Verified formulary item</small>
                                )}
                              </div>
                            </div>
                          </td>
                          <td>
                            <strong>{medicine.common_strength || 'Not specified'}</strong>
                            <small>{medicine.dosage_form || 'Form not specified'}</small>
                          </td>
                          <td>
                            <span
                              className={`medicine-type is-${medicine.rx_class?.toLowerCase()}`}
                            >
                              {medicine.rx_class === 'OTC' ? 'OTC' : 'Rx'}
                            </span>
                          </td>
                          <td>
                            <b className={`stock-count is-${status.key}`}>
                              {Number(medicine.stock_quantity || 0)}
                            </b>
                          </td>
                          <td>
                            <p className="medicine-description">
                              {medicine.short_description || 'No description available.'}
                            </p>
                          </td>
                          <td>
                            <span className={`stock-status is-${status.key}`}>
                              {status.key === 'in' ? (
                    ...
                    <tr>
                      <td colSpan="8">
                        <div className="admin-medicine-empty">
                          <Boxes size={34} />
            ...
            <footer>
              ...
              <nav aria-label="Medicine table pages">
                <button
                  disabled={page === 1}
        ...
        {drawer && (
          <aside className="admin-medicine-drawer">
            <header>
              <div>
                <span>
                  <Pill size={20} />
                ...
                <div>
                  <small>{drawer === 'edit' ? 'UPDATE FORMULARY' : 'NEW FORMULARY ITEM'}</small>
                  <h3>{drawer === 'edit' ? 'Edit medicine' : 'Add medicine'}</h3>
                </div>
              </div>
              <button onClick={() => setDrawer(null)} type="button" aria-label="Close form">
                <X size={19} />
              </button>
            </header>
            <form onSubmit={saveMedicine}>
              ...
              <div className="form-row">
                <label>
                  ...
                  <input
                    name="common_strength"
                    value={form.common_strength}
                    onChange={updateField}
                ...
                <label>
                  ...
                  <select name="dosage_form" value={form.dosage_form} onChange={updateField}>
              ...
              <div className="form-row">
                <label>
                  ...
                  <select name="rx_class" value={form.rx_class} onChange={updateField}>
              ...
              <label>
                ...
                <textarea
                  ...
                  onChange={updateField}
                  placeholder="Brief purpose and medicine description"
                  rows="5"
                  required
                />
              </label>
              <aside>
                <Info size={18} />
                <p>
                  <b>Stock status updates automatically.</b> 0 is out of stock, 1–10 is low stock,
                  and more than 10 is in stock. New entries require pharmacist verification.
                </p>
              </aside>
              <footer>
                <button type="submit" className="primary" disabled={saving}>
                  <Save size={17} />
                  {saving ? 'Saving…' : drawer === 'edit' ? 'Save changes' : 'Save medicine'}
                </button>
                <button type="button" onClick={() => setDrawer(null)} disabled={saving}>
                  Cancel
                </button>
              </footer>
            </form>
      ...
      {deleteTarget && (
        <div className="admin-medicine-confirm-backdrop">
          <section
            className="admin-medicine-confirm"
            role="alertdialog"
            aria-modal="true"
          ...
          >
            ...
            <h3 id="delete-medicine-title">Delete {deleteTarget.generic_name}?</h3>
            <p>
              The medicine can only be deleted if no patient record currently uses it. Otherwise,
              set the stock to 0.
            </p>
            <div>
              <button onClick={() => setDeleteTarget(null)} type="button" disabled={saving}>
                Keep medicine
              </button>
              <button className="danger" onClick={deleteMedicine} type="button" disabled={saving}>
