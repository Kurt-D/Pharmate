...
ALTER TABLE pharmacists
  ADD COLUMN IF NOT EXISTS license_jurisdiction VARCHAR(100) NULL AFTER license_number,
  ADD COLUMN IF NOT EXISTS license_status ENUM('PENDING','VERIFIED','SUSPENDED','EXPIRED')
    NOT NULL DEFAULT 'PENDING' AFTER license_jurisdiction,
  ADD COLUMN IF NOT EXISTS license_expires_on DATE NULL AFTER license_status,
  ADD COLUMN IF NOT EXISTS license_evidence_url VARCHAR(1000) NULL AFTER license_expires_on,
  ADD COLUMN IF NOT EXISTS license_verified_at DATETIME(3) NULL AFTER license_evidence_url,
  ADD COLUMN IF NOT EXISTS license_verified_by CHAR(36) NULL AFTER license_verified_at,
...
ALTER TABLE clinical_rule_revisions
  ADD COLUMN IF NOT EXISTS reviewer_license_number VARCHAR(100) NULL AFTER reviewed_by,
  ADD COLUMN IF NOT EXISTS reviewer_license_jurisdiction VARCHAR(100) NULL
    AFTER reviewer_license_number,
  ADD COLUMN IF NOT EXISTS reviewer_license_expires_on DATE NULL
    AFTER reviewer_license_jurisdiction;

ALTER TABLE prescription_photos
  ADD COLUMN IF NOT EXISTS reviewer_license_number VARCHAR(100) NULL AFTER pharmacist_id,
  ADD COLUMN IF NOT EXISTS reviewer_license_jurisdiction VARCHAR(100) NULL
    AFTER reviewer_license_number;

ALTER TABLE medications
  ADD COLUMN IF NOT EXISTS max_daily_doses_snapshot TINYINT UNSIGNED NULL AFTER frequency_code,
  ADD COLUMN IF NOT EXISTS min_interval_hours_snapshot DECIMAL(5,2) NULL
    AFTER max_daily_doses_snapshot,
  ADD COLUMN IF NOT EXISTS dose_limit_basis ENUM('CLINICAL_RULE','REFERENCE_LABEL','PATIENT_LABEL','PRESCRIPTION_DIRECTIONS')
