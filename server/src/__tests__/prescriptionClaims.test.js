...
async function login(email) {
  return (await request(app).post('/api/auth/login').send({ email, password: PASSWORD })).body
    .accessToken;
}
...
beforeAll(async () => {
  ...
  await request(app)
    ...
    .send({ email: patientEmail, password: PASSWORD, role: 'patient', full_name: 'Claim Patient' });
  ...
  pharm1Id = await createPrivilegedTestUser({
    email: p1Email,
    password: PASSWORD,
    role: 'pharmacist',
    fullName: 'One',
  });
  pharm2Id = await createPrivilegedTestUser({
    email: p2Email,
    password: PASSWORD,
    role: 'pharmacist',
    fullName: 'Two',
...
describe('validation claims', () => {
  test('first claim is atomic, owner retry is idempotent, and competitor learns nothing', async () => {
    ...
    expect(JSON.stringify(one.status === 409 ? one.body : two.body)).not.toMatch(
      /pharmacist|@test|claimed_by/i
    );
  });
  ...
  test('expired claim can be taken over and produces an expiry/reclaim audit sequence', async () => {
    ...
    await pool.execute(
      'UPDATE prescription_photos SET claim_expires_at=DATE_SUB(NOW(3), INTERVAL 1 SECOND) WHERE id=?',
      [photoId]
    );
    ...
    const history = await request(app)
      .get(`/api/pharmacist/validations/${photoId}/history`)
      .set(auth(pharm2Token));
    expect(history.body.history.map((event) => event.event_type)).toEqual([
      'claimed',
      'claim_expired',
      'reclaimed',
    ]);
    expect(JSON.stringify(history.body)).not.toMatch(
      /patient|medication|pharmacist_id|Claim Patient/i
    );
  });
  ...
  test('non-owner cannot release or view photo; active foreign claims are hidden from queue', async () => {
    ...
    expect(
      (
        await request(app)
          .delete(`/api/pharmacist/validations/${photoId}/claim`)
          .set(auth(pharm2Token))
      ).status
    ).toBe(409);
    expect(
  ...
  test('decision auto-claims an unclaimed item and notification is idempotent', async () => {
    ...
    const [[notifications]] = await pool.execute(
      ...
      [`prescription:${photoId}:approved`]
    );
    expect(Number(notifications.count)).toBe(1);
    const [events] = await pool.execute(
      'SELECT event_type FROM prescription_validation_audit WHERE prescription_id=? ORDER BY event_time,id',
      [photoId]
    );
  ...
  test('exactly one concurrent competing decision succeeds', async () => {
    ...
    const [one, two] = await Promise.all([
      ...
      request(app)
        .post('/api/pharmacist/validate')
        .set(auth(pharm2Token))
        .send({ photo_id: photoId, action: 'reject', reason: 'Not acceptable' }),
    ]);
    expect([one.status, two.status].sort()).toEqual([200, 409]);
    const [[decisions]] = await pool.execute(
      "SELECT COUNT(*) count FROM prescription_validation_audit WHERE prescription_id=? AND event_type IN ('approved','rejected','needs_clearer')",
      [photoId]
    );
      ...
      expect(
        (
          await request(app)
            .post('/api/pharmacist/validate')
            .set(auth(pharm1Token))
            .send({ photo_id: photoId, action, reason })
        ).status
      ).toBe(400);
    }
    expect(
      (
        await request(app)
          .post('/api/pharmacist/validate')
          .set(auth(pharm1Token))
          .send({ photo_id: photoId, action: 'reject', reason: 'x'.repeat(501) })
      ).status
    ).toBe(400);
  });
  ...
  test('patient and admin roles cannot access validation operations or history', async () => {
    ...
    for (const token of [patientToken, adminToken]) {
      expect(
        (await request(app).post(`/api/pharmacist/validations/${photoId}/claim`).set(auth(token)))
          .status
      ).toBe(403);
      expect(
        (await request(app).get(`/api/pharmacist/validations/${photoId}/history`).set(auth(token)))
          .status
      ).toBe(403);
      expect(
        (
  ...
  test('rolls back claim, decision, notification, medication, and audit on transaction failure', async () => {
    ...
    await expect(
      decideValidation(pharm1Id, photoId, 'approve', null, { failBeforeCommit: true })
    ).rejects.toThrow(/Injected/);
    const [[photo]] = await pool.execute(
      'SELECT status,claimed_by FROM prescription_photos WHERE id=?',
      [photoId]
    );
    const [[med]] = await pool.execute('SELECT status FROM medications WHERE id=?', [medicationId]);
    const [[audits]] = await pool.execute(
      'SELECT COUNT(*) count FROM prescription_validation_audit WHERE prescription_id=?',
      [photoId]
    );
