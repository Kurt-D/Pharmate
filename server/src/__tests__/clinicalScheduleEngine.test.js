...
const rule = (overrides = {}) => ({
  ...
  generic_name: 'Example medicine', default_strength: '50 mg', dosage_form: 'tablet',
  standard_frequency: 'BID', food_rule: 'WITH_MEAL', min_interval_hours: 0,
  max_daily_doses: 2, clinical_rule_status: 'VERIFIED', food_instruction: 'Take with meals',
  ...overrides,
});
...
describe('automated clinical schedule engine', () => {
  ...
  test('groups different medicines assigned to the same time', () => {
    const result = generateClinicalSchedule([
      ...
      rule({ drug_id: '00000000-0000-4000-8000-000000000002', generic_name: 'Second medicine', standard_frequency: 'QD' }),
    ]);
  ...
  test('deduplicates the same active ingredient with a warning', () => {
    ...
    expect(result.warnings).toEqual(expect.arrayContaining([
      expect.objectContaining({ code: 'DUPLICATE_ACTIVE_INGREDIENT' }),
    ]));
  ...
  test('honors the verified daily dose cap for q4h rules', () => {
    const result = generateClinicalSchedule([rule({ standard_frequency: 'q4h', max_daily_doses: 4, min_interval_hours: 4 })]);
  ...
  test('uses evenly spaced times when BID requires a 12-hour minimum gap', () => {
    const result = generateClinicalSchedule([rule({
      food_rule: 'NONE', min_interval_hours: 12,
    })]);
  ...
  test('keeps exact hourly gaps when a first-dose preference is supplied', () => {
    const result = generateClinicalSchedule([rule({
      standard_frequency: 'Q8H', max_daily_doses: 3, min_interval_hours: 8,
      first_dose_time: '07:00', food_rule: 'NONE',
    })]);
