...
beforeAll(async () => {
  await createPrivilegedTestUser({
    email, password: PASSWORD, role: 'pharmacist', fullName: 'Clinical Rule Reviewer',
  });
  token = (await request(app).post('/api/auth/login').send({ email, password: PASSWORD })).body.accessToken;
  ...
  await pool.execute(
    `UPDATE drug_reference SET common_strength='10 mg',dosage_form='tablet',
     catalog_status='VERIFIED',clinical_rule_status='UNVERIFIED' WHERE id=?`, [drugId]
  );
});
...
test('complete rule is verified and creates an immutable revision', async () => {
  const response = await request(app)
    ...
    .set(auth())
    .send({
      action: 'VERIFY', common_strength: '10 mg', dosage_form: 'tablet',
      administration_route: 'ORAL', release_type: 'IMMEDIATE_RELEASE',
      supported_frequency_codes: ['QD'],
      frequency_default: 'QD', max_daily_doses: 1, min_interval_hours: 24,
  ...
  expect(history.body[0]).toMatchObject({ action: 'VERIFIED', rule_version: response.body.rule_version });
});
