...
const report = { read: rows.length, inserted_provisional: 0, updated_provisional: 0, preserved_verified: 0, rejected: 0 };
const conn = await pool.getConnection();
try {
  await conn.beginTransaction();
  for (const row of rows) {
    const name = String(row.generic_name || '').trim().toLowerCase();
    if (!name || !['RX', 'OTC'].includes(row.rx_class)) { report.rejected++; continue; }
    ...
    const [[existing]] = await conn.execute(
      `SELECT id,is_provisional FROM drug_reference
       ...
       ORDER BY is_provisional ASC, created_at ASC LIMIT 1`, [name]
    );
    if (existing && !existing.is_provisional) { report.preserved_verified++; continue; }
    const values = [row.min_interval_hours ?? null, row.max_daily_doses ?? null,
      row.is_prn_default ? 1 : 0, row.default_interval_hours ?? null,
      row.meal_anchor_code || 'NONE', row.meal_instruction ?? null,
      row.standard_frequency ?? null, row.notes ?? null, row.rx_class];
    if (existing) {
      await conn.execute(
        `UPDATE drug_reference SET min_interval_hours=?,max_daily_doses=?,is_prn_default=?,
         ...
         notes=?,rx_class=?,is_provisional=1 WHERE id=?`, [...values, existing.id]
      );
