...
function parseCsv(text) {
  ...
  return rows.filter((cells) => cells.some(Boolean)).map((cells) => Object.fromEntries(headers.map((header, index) => [header, cells[index]?.trim() ?? ''])));
}
...
try {
  ...
  for (const row of rows) {
    ...
    if (!name || seen.has(name)) { report.rejected++; continue; }
    seen.add(name);
    const rxClass = row.rx_otc_status_ph_reference.toUpperCase() === 'OTC' ? 'OTC' : 'RX';
    const [[existing]] = await conn.execute(
      `SELECT id,is_provisional FROM drug_reference
       WHERE LOWER(TRIM(generic_name))=? ORDER BY is_provisional ASC,created_at ASC LIMIT 1`, [name]
    );
    const metadata = [row.therapeutic_category || null,row.drug_class || null,row.common_uses_conditions || null,row.short_description || null,row.common_strength || null,row.dosage_form || null,path.basename(input)];
    if (existing) {
      await conn.execute(
        `UPDATE drug_reference SET therapeutic_category=?,drug_class=?,common_uses=?,short_description=?,
         ...
         WHERE id=?`, [...metadata,rxClass,existing.id]
      );
      if (existing.is_provisional) report.updated++; else report.preserved_verified++;
    } else {
      await conn.execute(
