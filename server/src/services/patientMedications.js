...
export function validateMedicationPatch(body) {
  ...
  if (forbidden.length)
    return fail(
      400,
      'Forbidden medication fields',
      `Patients cannot change: ${forbidden.join(', ')}`
    );
  if (!('expected_updated_at' in body)) return fail(400, 'expected_updated_at is required');
  if (Number.isNaN(Date.parse(body.expected_updated_at)))
    return fail(400, 'expected_updated_at must be an ISO date-time');
  if (!keys.some((key) => EDITABLE.has(key)))
    return fail(400, 'No editable medication fields provided');
  const value = {};
  if ('dosage_instruction' in body) {
    if (body.dosage_instruction !== null && typeof body.dosage_instruction !== 'string')
      return fail(400, 'dosage_instruction must be text or null');
  ...
  if ('frequency' in body) {
    if (typeof body.frequency !== 'string' || !body.frequency.trim())
      return fail(400, 'frequency must be non-empty text');
    if (body.frequency.length > 255) return fail(400, 'frequency is too long');
    const code = parseFrequency(body.frequency);
    if (code === 'CONSULT')
      return fail(400, 'frequency is not supported; pharmacist review is required');
  ...
  for (const key of ['start_date', 'end_date']) {
    if (key in body) {
      if (body[key] !== null && (typeof body[key] !== 'string' || !validDate(body[key])))
        return fail(400, `${key} must be YYYY-MM-DD or null`);
      value[key] = body[key];
    }
  }
...
export async function updateMedication(patientId, medicationId, parsed) {
  ...
  try {
    ...
    const [[row]] = await conn.execute(`${SELECT} WHERE m.id=? AND m.patient_id=? FOR UPDATE`, [
      medicationId,
      patientId,
    ]);
    if (!row) {
    ...
    if (!expectedMatches(parsed.expected, row.updated_at)) {
      await conn.rollback();
      return fail(
        409,
        'Medication changed since it was loaded',
        'Reload the medication and try again.'
      );
    }
    if (row.source !== 'OTC_SELF') {
      ...
      return fail(
        ...
        'Validated prescription dosage and frequency cannot be changed by a patient. Contact a pharmacist for clinical changes.'
      );
    }
    if (['completed', 'cancelled'].includes(row.status)) {
      await conn.rollback();
      return fail(409, 'Stopped medications cannot be edited');
    }
    if (
      parsed.value.frequency_code &&
      row.max_daily_doses != null &&
      dosesPerDay(parsed.value.frequency_code) > Number(row.max_daily_doses)
    ) {
      await conn.rollback();
    ...
    if (timingChanged)
      await createPatientNotification({
        ...
        eventKey: `medication:${row.id}:changed:${uuidv4()}`,
        metadata: { medication_id: row.id },
        executor: conn,
      });
    await conn.commit();
    return {
      medication: medication(updated),
      schedule_reconfirmation_required: timingChanged,
      future_schedules_invalidated: invalidated,
    };
  } catch (error) {
    await conn.rollback();
    throw error;
...
export async function stopMedication(patientId, medicationId, expected) {
  if (!expected || Number.isNaN(Date.parse(expected)))
    return fail(400, 'expected_updated_at is required and must be an ISO date-time');
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const [[row]] = await conn.execute(`${SELECT} WHERE m.id=? AND m.patient_id=? FOR UPDATE`, [
    ...
    if (['completed', 'cancelled'].includes(row.status)) {
      ...
      return {
        ...
        future_schedules_invalidated: 0,
      };
    }
    if (!expectedMatches(expected, row.updated_at)) {
      await conn.rollback();
      return fail(
        409,
        'Medication changed since it was loaded',
    ...
    const before = auditSnapshot(row);
    await conn.execute('UPDATE medications SET status=?, updated_at=NOW(3) WHERE id=?', [
      nextStatus,
      row.id,
    ]);
    ...
    await insertAudit(
      ...
      row,
      patientId,
      nextStatus === 'completed' ? 'stopped' : 'cancelled',
      before,
...
export function parseHistoryQuery(query = {}) {
  ...
  if (!/^\d+$/.test(rawLimit) || Number(rawLimit) < 1 || Number(rawLimit) > 100)
    return fail(400, 'limit must be between 1 and 100');
  ...
  return {
    value: { limit: Number(rawLimit), eventType: query.event_type, status: query.status, cursor },
  };
}
...
export async function listMedicationHistory(patientId, options) {
  ...
  if (options.status) {
    where.push("JSON_UNQUOTE(JSON_EXTRACT(after_info, '$.status'))=?");
    params.push(options.status);
  }
  if (options.cursor) {
    where.push('(event_time < ? OR (event_time = ? AND id < ?))');
    params.push(options.cursor.time, options.cursor.time, options.cursor.id);
  }
  const [rows] = await pool.execute(
    `SELECT id, medication_id, actor_id, actor_role, event_type, before_info, after_info, event_time FROM medication_history WHERE ${where.join(' AND ')} ORDER BY event_time DESC, id DESC LIMIT ${options.limit + 1}`,
    params
  );
  ...
  return {
    ...
    pagination: {
      ...
      next_cursor: hasMore
        ? Buffer.from(
            JSON.stringify([new Date(page.at(-1).event_time).toISOString(), page.at(-1).id])
          ).toString('base64url')
        : null,
    },
  };
