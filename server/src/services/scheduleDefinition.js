...
export function normalizeTimes(values = []) {
  return [...new Set((Array.isArray(values) ? values : []).map(String).map((v) => v.slice(0, 5)).filter((v) => CLOCK.test(v)))].sort();
}
...
export function frequencyRule(value, intervalHours = null) {
  ...
  if (type === 'EVERY_N_HOURS') {
    ...
    return {
      ...
      count: Number.isInteger(interval) && interval > 0 && 24 % interval === 0 ? 24 / interval : null,
    };
  }
  return null;
}
...
export function suggestedTimes(value, startTime, intervalHours = null) {
  ...
  if (!rule || rule.count === 0 || !Number.isInteger(rule.count) || !CLOCK.test(startTime || '')) return [];
...
export function validateMedicationSchedule({ frequencyType, frequencyCode, intervalHours, scheduleTimes = [], startDate, endDate } = {}) {
  const rule = frequencyRule(frequencyType || frequencyCode, intervalHours);
  if (!rule) return { valid: false, code: 'UNSUPPORTED_FREQUENCY', message: 'Choose a supported medication frequency.' };
  if (!startDate || (endDate && endDate < startDate)) return { valid: false, code: 'INVALID_DATE_RANGE', message: 'The medication date range is invalid.' };
  if (rule.frequencyType === 'AS_NEEDED') return { valid: scheduleTimes.length === 0, code: scheduleTimes.length ? 'AS_NEEDED_HAS_FIXED_TIMES' : null, message: scheduleTimes.length ? 'As-needed medication cannot have active fixed reminder times.' : null };
  const times = normalizeTimes(scheduleTimes);
  if (times.length !== scheduleTimes.length) return { valid: false, code: 'INVALID_OR_DUPLICATE_TIME', message: 'Reminder times must be valid and unique.' };
  if (!Number.isInteger(rule.count)) return { valid: false, code: 'INVALID_INTERVAL', message: 'The interval must divide evenly into 24 hours.' };
  if (times.length !== rule.count) {
    const labels = { 1: 'Once-daily medication requires one scheduled dose per day.', 2: 'Twice-daily medication requires two scheduled doses per day.', 3: 'Three-times-daily medication requires three scheduled doses per day.' };
    return { valid: false, code: 'FREQUENCY_TIME_COUNT_MISMATCH', message: labels[rule.count] || `${rule.frequencyType === 'EVERY_N_HOURS' ? `Every-${rule.intervalHours}-hours` : 'This'} medication schedule requires ${rule.count} scheduled doses per 24-hour cycle.` };
  }
  return { valid: true, code: null, message: null, expectedCount: rule.count };
}

function timesInInstructions(...values) {
  const matches = values.flatMap((value) => String(value || '').match(/\b(?:[01]\d|2[0-3]):[0-5]\d\b/g) || []);
...
export function deriveScheduleDefinition(input, frequencyCode) {
  ...
  const intervalStartTime = String(input.interval_start_time || input.first_dose_time || input.start_time || '').slice(0, 5);
  const validStart = CLOCK.test(intervalStartTime) ? intervalStartTime : null;
  const days = Array.isArray(input.schedule_days) ? [...new Set(input.schedule_days.map(String))] : [];

  if (scheduleType === 'AS_NEEDED') {
    return { scheduleType, times: [], intervalHours: null, intervalStartTime: null, days, status: 'APPROVED' };
  }
  if (times.length) {
    return { scheduleType: requestedType || 'SPECIFIC_TIMES', times, intervalHours: null, intervalStartTime: null, days, status: 'PENDING_APPROVAL' };
  }
  if (scheduleType === 'EVERY_N_HOURS' && intervalHours >= 1 && intervalHours <= 24 && validStart) {
    return { scheduleType, times: [], intervalHours, intervalStartTime: validStart, days, status: 'PENDING_APPROVAL' };
  }
  return { scheduleType: scheduleType || null, times: [], intervalHours, intervalStartTime: validStart, days, status: 'NEEDS_REVIEW' };
}
