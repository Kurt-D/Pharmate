...
const LABEL_FREQUENCIES = new Set(['QD', 'BID', 'TID', 'QID', 'Q4H', 'Q6H', 'Q8H', 'Q12H', 'BEDTIME', 'PRN', 'OTHER']);
const LABEL_FOOD_INSTRUCTIONS = new Set(['NONE', 'WITH_MEAL', 'BEFORE_MEAL', 'AFTER_MEAL', 'EMPTY_STOMACH']);
const FORM_ALIASES = new Map([
  ['tablet', 'tablet'], ['tablets', 'tablet'], ['capsule', 'capsule'], ['capsules', 'capsule'],
  ['syrup', 'syrup'], ['suspension', 'suspension'], ['solution', 'solution'],
  ['eye drops', 'eye drops'], ['eye drop', 'eye drops'], ['ear drops', 'ear drops'],
  ['ear drop', 'ear drops'], ['inhaler', 'inhaler'], ['injection', 'injection'],
  ['cream', 'topical'], ['ointment', 'topical'], ['topical', 'topical'],
  ['patch', 'patch'],
]);
...
export function normalizeDosageForm(value) {
  return FORM_ALIASES.get(String(value || '').trim().toLowerCase()) || String(value || '').trim().toLowerCase();
}

export function parseStrength(value) {
  const match = String(value || '').trim().match(/(\d+(?:\.\d+)?)\s*(mcg|mg|g|ml|mL|units?|iu)\b/i);
...
export function validateIntakeRecord(record, verifiedDrug, { allowExistingStart = false } = {}) {
  ...
  const customStrength = String(record?.custom_strength || verifiedDrug?.default_strength || verifiedDrug?.common_strength || '').trim();
  ...
  const labelFrequency = String(record?.label_frequency || '').trim().toUpperCase();
  const labelFoodInstruction = String(record?.label_food_instruction || 'NONE').trim().toUpperCase();
  const entryMethod = String(record?.entry_method || 'MANUAL').toUpperCase();
  const confidence = record?.ocr_confidence === null || record?.ocr_confidence === undefined
    ? null : Number(record.ocr_confidence);
  ...
  try {
    const brands = typeof verifiedDrug.brand_names_json === 'string'
      ? JSON.parse(verifiedDrug.brand_names_json) : verifiedDrug.brand_names_json;
  ...
  if (!acceptedNames.some((name) => canonical(name) === canonical(medicineName))) {
    return { error: `Confirm ${verifiedDrug.generic_name} from the verified medicine suggestions.` };
  }
  if (!dosageInstruction || dosageInstruction.length > 255) {
    return { error: `Enter the dosage exactly as shown on the label for ${verifiedDrug.generic_name}.` };
  }
  ...
  if (!strength.value || !strength.unit) {
    return { error: `Enter the strength shown on the label for ${verifiedDrug.generic_name}, including its unit.` };
  }
  ...
  if (!LABEL_FREQUENCIES.has(labelFrequency)) {
    return { error: `Choose how often the label or prescription says to take ${verifiedDrug.generic_name}.` };
  }
  if (labelFrequency === 'OTHER' && !labelDirection) {
    return { error: `Enter the other label or prescription instructions for ${verifiedDrug.generic_name}.` };
  }
  if (!LABEL_FOOD_INSTRUCTIONS.has(labelFoodInstruction)) {
    return { error: `Choose the food instruction printed on the label for ${verifiedDrug.generic_name}.` };
  }
  if (!allowExistingStart && startDate < manilaToday()) {
    return { error: `The schedule start date for ${verifiedDrug.generic_name} cannot be in the past.` };
  }
  ...
  if (record?.patient_confirmed !== true) {
    return { error: `Review and confirm the medicine information for ${verifiedDrug.generic_name}.` };
  }
  const endDate = String(record?.end_date || '').trim();
  if (endDate && (!DATE_RE.test(endDate) || Number.isNaN(Date.parse(`${endDate}T00:00:00Z`)) || endDate < startDate)) {
    return { error: `Choose an end date on or after the start date for ${verifiedDrug.generic_name}.` };
  }
  let brandName = null;
  try {
    const brands = typeof verifiedDrug.brand_names_json === 'string'
  ...
  return {
    value: {
      ...
      purpose: String(record?.purpose || '').trim().slice(0, 255) || null,
      release_type_snapshot: String(record?.release_type_snapshot || '').trim().slice(0, 80) || null,
