...
function treatmentDatesForMedicine(medicine) {
  ...
  return dates.filter((date) => {
    ...
    return allowed.has(String(weekday)) || allowed.has(names[weekday]);
  });
}
...
function mealAnchoredFrequency(frequencyCode, mealInstruction = '') {
  ...
  const modifier = /before/.test(instruction) ? ':AC' : /after/.test(instruction) ? ':PC' : '';
  if (/evening meal|dinner/.test(instruction) && frequencyCode === 'QD')
    return `MEALMAP(0,0,1)${modifier}`;
  if (/lunch|noon/.test(instruction) && frequencyCode === 'QD') return `MEALMAP(0,1,0)${modifier}`;
...
export async function loadEngineInput(patientId) {
  ...
  const medications = meds.map((m) => {
    ...
    return {
      ...
      scheduleTimes: jsonArray(m.schedule_times),
      intervalHours: m.interval_hours != null ? Number(m.interval_hours) : null,
...
export function classifyScheduleSafety(input, result, targetMedicationIds = []) {
  ...
  for (const med of input.medications) {
    ...
    if (!med.drugId || (enforceVerifiedRules && !med.isVerified)) {
      findings.push(
        finding({
          code: 'UNVERIFIED_MEDICINE_RULE',
          level: 'unavailable',
          title: 'Verified timing rule unavailable',
          message: NO_VERIFIED_RULE,
          medicines: [med.id],
        })
      );
    }
  ...
  for (const rule of input.interactions) {
    ...
    if (!rule.isVerified) {
      if (enforceVerifiedRules)
        findings.push(
          finding({
            code: 'UNVERIFIED_INTERACTION_RULE',
            level: 'unavailable',
      ...
      findings.push(
        finding({
          code: 'VERIFIED_SPACING_RULE',
          level: 'adjusted',
          title: 'Verified time gap',
  ...
  for (const requestedId of targetIds) {
    if (!input.medications.some((medicine) => String(medicine.id) === requestedId)) {
      findings.push(
        finding({
          code: 'UNKNOWN_SELECTED_MEDICINE',
          level: 'unavailable',
          title: 'Verified recommendation unavailable',
          message:
            'The selected medicine is no longer active. Return to your medicine list and choose it again.',
...
export async function proposeForPrescription(patientId, medicationId) {
  ...
  const target = meds.find((medicine) => medicine.id === medicationId);
  let exactTimes = [];
  try {
...
export async function proposeForPatient(patientId, targetMedicationIds = []) {
  ...
  const medications = orderedMedicines.filter((medicine) => {
    const key = [
      ...
      String(medicine.dosageInstruction || '')
        .trim()
        .toLowerCase(),
      String(medicine.frequencyCode || medicine.frequency || '')
        .trim()
        .toLowerCase(),
    ].join('|');
  ...
  const explicitSlots = medications.flatMap((medicine) => {
    ...
    return medicine.scheduleTimes.map((time) => {
      ...
      return {
        ...
        reason: 'Exact medication time entered from the prescription or by an authorized user',
      };
    });
  });
  result.slots.push(...explicitSlots);
  for (const medicine of medications) {
    if (medicine.isPrn || medicine.scheduleType === 'AS_NEEDED') continue;
    if (!result.slots.some((slot) => slot.medicationId === medicine.id)) {
      result.unresolved.push({
        medicationId: medicine.id,
        drugName: medicine.drugName,
        reason: 'Exact reminder times are not specified. This schedule requires review.',
      });
    }
  }
...
export async function analyzeAdjustedLayout(patientId, doses) {
  const targetMedicationIds = [
    ...new Set((doses || []).map((dose) => dose.medication_id).filter(Boolean)),
  ];
  ...
  if (!Array.isArray(doses) || !doses.length) {
    return {
      ...safety,
      can_save: false,
      validation_error: 'At least one reminder time is required.',
    };
  }
  for (let index = 0; index < doses.length; index += 1) {
    ...
    if (!result.ok) {
      safety.findings.push(
        finding({
          code: 'MANUAL_TIME_CONFLICT',
          level: 'manual',
          title: 'Manual review needed',
          message:
            result.error === 'unknown_medication'
              ? NO_VERIFIED_RULE
 ...
 */
export async function confirmForPatient(
  patientId,
  adjusted,
  targetMedicationIds = [],
  options = {}
  ...
  if (Array.isArray(adjusted) && adjusted.length > 0) {
    ...
    slots = adjusted.flatMap((dose) => {
      ...
      const dates =
        Array.isArray(dose.dates) && dose.dates.length
  ...
  try {
    ...
    if (affectedIds.length) {
      ...
      await conn.execute(
        `UPDATE medications
            SET schedule_status='APPROVED', schedule_approved_by=?, schedule_approved_at=NOW(3),
                schedule_updated_by=?, schedule_updated_at=NOW(3)
          WHERE patient_id=? AND id IN (${affectedPlaceholders})`,
