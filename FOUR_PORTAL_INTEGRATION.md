...
| Patient    | Inquiries          | Enable / withdraw inquiry consent     | `GET`, `POST`, `DELETE /api/patient/inquiry-consent`                                                     | Own patient account; explicit current-version acceptance | Central consent status and audit history        |
| Patient    | Inquiries          | Submit / message / complete / history | Patient inquiry routes, `INQUIRY_UPDATED`                                                               | Own inquiry; current consent required for new content | Central thread and messages retained after completion |
...
| Caregiver  | Orders / inquiry   | Submit request                        | Caregiver refill, delivery, inquiry routes                                                              | Active caregiver link; inquiry also requires patient's current consent | Shared order or central inquiry record    |
...
| Pharmacist | Inquiries          | Preview / accept / reply / close      | Pharmacist inquiry routes, `INQUIRY_UPDATED`                                                            | Eligible routing for preview/accept; assigned pharmacist for transcript; patient's current consent for new messages | Central history retained after closure, audit event |
...
- Inquiry subjects and messages are centrally stored and may contain identifying
  information. The queue uses a patient code, which is a pseudonym; it also
  exposes the subject and priority metadata to eligible pharmacists before
  assignment. Full transcripts require owning-patient or assigned-pharmacist
  access. Inquiry text is not application-layer encrypted.
- Inquiry policy version `2026-09-06` requires optional, explicit patient consent.
  Withdrawal blocks new inquiries and messages without removing old records;
  closing a conversation is not deletion. There is no automatic inquiry purge
  or self-service erasure endpoint. See the
  [inquiry privacy contract](docs/INQUIRY_PRIVACY_POLICY.md) for retention,
  infrastructure access, and local-label distinctions.
